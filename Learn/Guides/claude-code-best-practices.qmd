...
::: {.callout-tip}
## New to agentic coding? Start with the workshop
Our tool-agnostic **[Intro to Agentic Coding](../Workshops/Intro-Agentic-Coding.qmd)** workshop (developed for MLM26) covers the core principles behind this guide: limiting what the agent can access, planning, feature-based development, and verifying everything. This guide goes deeper on the Claude Code specifics.
:::
...
- **Built-in sandboxing** — an [OS-level sandbox](https://code.claude.com/docs/en/sandboxing) restricts filesystem access to your project directory and limits outbound network traffic. **This is the single most impactful security measure you can enable.**
...
- **Web version** (claude.ai/code) — the most restricted surface. Each task runs in a fresh, ephemeral VM with [Gvisor-based kernel isolation](https://gvisor.dev/); storage is wiped when the task completes and credentials never exist inside the sandbox.

See [Security fundamentals](#security-fundamentals) below for configuration details, deny-rule examples, and container guidance — or the [Cloud Setup Guide's security section](../Guides/claude-code-cloud-setup.qmd#understand-risk) for a step-by-step walkthrough.
...
- **Aim for under 300 lines.** Some teams keep theirs [under 60](https://www.humanlayer.dev/blog/writing-a-good-claude-md). There's no hard limit, but shorter files mean less context pollution.
...
- **Move specialized knowledge into [skills](https://code.claude.com/docs/en/skills).** Skills load on demand — a `/deploy` skill or a `migration-conventions` skill only enters context when relevant, keeping your base `CLAUDE.md` lean.

The bottom line: write `CLAUDE.md` for the *model*, not for humans. Keep it concise, universally applicable, and structured. As [Anthropic has noted](https://platform.claude.com/docs/en/build-with-claude/context-windows), larger context windows won't solve this — "context pollution and information relevance concerns" apply at any window size.
:::

See the [official `CLAUDE.md` reference](https://code.claude.com/docs/en/memory) for the full spec, including file resolution order and advanced features.
