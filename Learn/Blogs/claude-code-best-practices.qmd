...
The [Claude Desktop app](https://claude.ai/download) includes three tabs: **Chat** (standard conversation, with [MCP](https://modelcontextprotocol.io/) support), **Code** (the full Claude Code agentic experience — see below), and **Cowork** (an autonomous agent for knowledge work — it can execute multi-step tasks on your desktop, like research, file organization, and document creation). Only the **Code** tab is an agentic coding tool. The Chat tab provides the same experience as [claude.ai](https://claude.ai) in a native app.
:::
...
Anthropic reports that the average Claude Code user on API pricing spends [roughly $6/day](https://code.claude.com/docs/en/costs), with 90% of users under $12/day. That translates to **$100–200/month** for active development with Sonnet. But averages hide a lot of variance — one developer documented a single session that [hit ~$150/hour](https://jeffry.in/expensive-ai/) running multiple parallel agents. One detailed [usage report](https://ccusage.com/guide/monthly-reports) showed ~892K output tokens vs ~45K input tokens in a single month on a mix of Opus and Sonnet, costing ~$1,248.
...
**Don't stack them.** The built-in sandbox and Docker containers are alternative isolation strategies. Running bubblewrap inside Docker introduces [nested sandbox complexity](https://code.claude.com/docs/en/sandboxing) without meaningful security benefit. Pick one: sandbox for interactive work, containers for headless automation.

### Watch for prompt injection and runaway agents

**Prompt injection** is when an agent reads a file or message that contains hidden instructions designed to hijack its behavior. A malicious `README.md`, issue body, or [`.docx` attachment](https://www.promptarmor.com/resources/claude-cowork-exfiltrates-files) could trick the agent into exfiltrating files or running harmful commands. Be especially cautious when pointing an agent at untrusted repositories or external content. Deny rules and sandboxing are your main defenses here — they limit what the agent can do even if it's been tricked.
