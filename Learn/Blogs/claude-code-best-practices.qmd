...
Options include a plain [Docker container](https://docs.docker.com/ai/sandboxes/agents/claude-code/) with your project mounted as a volume, [Docker sandboxes](https://www.docker.com/blog/docker-sandboxes-run-claude-code-and-other-coding-agents-unsupervised-but-safely/) (microVM-based isolation), or cloud sandbox platforms like [E2B](https://e2b.dev/). For CI pipelines, ephemeral containers that are destroyed after each run are the safest option — nothing persists between runs.

Note: the built-in sandbox and Docker containers are **alternative** isolation strategies, not layers to stack. Running bubblewrap inside Docker introduces [nested sandbox complexity](https://code.claude.com/docs/en/sandboxing) without meaningful security benefit.
