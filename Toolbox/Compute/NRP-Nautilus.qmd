...
- [**CHTC**](CHTC.qmd) — free GPU Lab (A100s, H100s, H200s) for batch training, fine-tuning, and parameter sweeps. Comfortably handles inference and fine-tuning of models up to ~70B with quantization on a single 80 GB GPU. Not a serving platform; no NVLink multi-GPU.
- [**UW Cloud Services**](UW-Cloud-Services.qmd) (AWS / GCP / Azure) — hyperscaler hardware (B200s, NVLink multi-GPU, world-class latency), with direct equivalents to NRP's offerings: hosted LLM APIs via [Amazon Bedrock](https://aws.amazon.com/bedrock/) or [Google's Gemini API](https://ai.google.dev/) (analog of NRP's LLM endpoint), and managed ML platforms — [SageMaker](https://aws.amazon.com/sagemaker/), [Vertex AI](https://cloud.google.com/vertex-ai), [Azure ML](https://azure.microsoft.com/en-us/products/machine-learning) — for notebooks and training (analog of NRP's JupyterHub / GPU pods). Also the route for persistent endpoints that stay up beyond HPC job time limits. Pay-per-hour, but UW-provisioned accounts cut grant overhead from 55.5% to 26%, get Internet2 NET+ pricing, and are HIPAA-eligible.
- [**GenAI at UW–Madison**](../GenAI/GenAI-at-UW-Madison.qmd) — proprietary frontier-model services (Claude, GPT-4o, Gemini, ChatGPT Edu) and policy guidance for sensitive data.
