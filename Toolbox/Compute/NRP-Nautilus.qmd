...
- [**CHTC**](CHTC.qmd) — free GPU Lab (A100s, H100s, H200s) for batch training, fine-tuning, and hyperparameter sweeps. Not a serving platform; no NVLink multi-GPU.
- [**UW Cloud Services**](UW-Cloud-Services.qmd) (AWS / GCP / Azure) — when you need persistent endpoints (RAG or serving apps that stay up beyond HPC job time limits), managed ML platforms (Vertex AI, SageMaker, Azure ML), NVLink multi-GPU, or the newest hardware (B200). Pay-per-hour, but UW-provisioned accounts cut grant overhead from 55.5% to 26%, get Internet2 NET+ pricing, and are HIPAA-eligible.
