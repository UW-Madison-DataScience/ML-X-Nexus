...
2. **A way to load the token into `os.environ["NRP_LLM_TOKEN"]`** without typing it into a notebook cell that gets saved or shared. Two paths:
   ...
   - **Google Colab** — Don't paste the token into a cell; Colab autosaves to Drive in real time and your token would ride along. Instead, use Colab's built-in **Secrets** panel (key icon in the left sidebar): click *Add new secret*, name it `NRP_LLM_TOKEN`, paste the value, and toggle "Notebook access." Then in the install cell, swap the `dotenv` block for:

     ```python
     from google.colab import userdata
     os.environ["NRP_LLM_TOKEN"] = userdata.get("NRP_LLM_TOKEN")
     ```
