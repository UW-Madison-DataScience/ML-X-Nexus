...
1. **An LLM token.** Visit [nrp.ai/llmtoken](https://nrp.ai/llmtoken) (you need to be in an LLM-flagged namespace — see [Get started](#get-started) above). The page has two text fields:
   - **Group:** type the name of the namespace this token should belong to (e.g., `wisc-ml-marathon`). Tokens are scoped to one namespace; if you later need one for a different group, you mint a new token.
   - **Alias:** any label for your own bookkeeping — NRP doesn't validate it or show it anywhere else. `dev` is a fine default; use something more specific (e.g., `endemann-laptop`, `nexus-card-demo`) if you'll have multiple tokens you want to tell apart and revoke independently.
